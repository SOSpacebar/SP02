...
void TScene::Init()
{
	...
	//Enable Camera
	camera.Init(Vector3(70, 5, 0), Vector3(0, 5, 0), Vector3(0, 1, 0));
	...
	//for (int i = 0; i < EnvironmentManager::orePos.size(); i++)
		_gameObjectMananger.add(GameObjectManager::objectType::T_ENVIRONMENTAL, new Vein(this, "ore", Vector3(0,0,0)));
	/*Vein::numOres = 3;
	Vein::init();
...
void TScene::Render()
{
	...
	//label
	_UIManager.renderTextOnScreen(UIManager::UI_Text("THIS IS TSCENE", Color(0, 1, 0), 3, .5f, 20));
	//FPS
...
//void TScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
//{
//	if (!mesh || mesh->textureID <= 0) //Proper error check
//		return;
//
